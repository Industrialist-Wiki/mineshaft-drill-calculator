import {type AcidId, Acids, type DepthId, Depths, type DepthYield, type DrillheadId, DrillHeads, type OilId, Oils, Resources,} from './data';
...
function dropdownButton(
    className: string, innerText: string, onClick: () => any) {
...
function changeDropdownShow(id: DropdownId, change: 'hide'|'toggle') {
  (Object.keys(Dropdowns) as DropdownId[]).forEach((dropdownId) => {
    if (dropdownId === id && change === 'toggle')
      ...
      Dropdowns[dropdownId].content.classList.remove('show');
  });
}
...
function recalculateYields() {
  ...
  Labels.deteriorationLabel.innerHTML =
      '⛓️‍💥 Deterioration Rate: ' + truncate(deteriorationRate) +
      '%/s';
  ...
  Labels.efficiencyLabel.innerHTML =
      `🔧 Efficiency: <b style='color: rgb(${(1 - efficiency) * 255}, ${
          efficiency * 255}, 0);'>${truncate(efficiency * 100)}%</b>`;

  Labels.drillHeadLabel.innerHTML =
      `<div class='${drillHead.className} squareDisplay drill'></div>${
          drillHead.infoText}: ${truncate(1 / cycleTime)}u/s (average), ${
          truncate(1 / lifeTime)}u/s (active)`;
  Labels.materialsLabel.innerHTML =
      `<div class='${drillHead.className} squareDisplay ingot'></div>${
          drillHead.materialInfoText}: ${
          truncate(drillHead.materialAmount / cycleTime)}u/s (average), ${
          truncate(drillHead.materialAmount / lifeTime)}u/s (active)`;
  Labels.acidLabel.innerHTML = `${acid.infoText}: ${
      truncate(acid.rate * efficiency)}L/s (average), ${acid.rate}L/s (active)`;
  Labels.oilLabel.innerHTML = `${oil.infoText}: ${
      truncate(oil.rate * (1 - replacementTime / cycleTime))}L/s (average), ${
      oil.rate}L/s (active)`;
  Labels.powerLabel.innerHTML = `⚡ Power: ${
      truncate(
          (0.1 * replacementTime + 0.5 * travelTime + 2 * lifeTime) /
          cycleTime)}MMF/s (average), 2.1MMF/s (active)`;

  function getInnerText(dy: DepthYield): string {
    const amount = dy.amount * (machineOil ? 1.1 : 1);
    return `${Resources[dy.resourceId]}: ${
        truncate(amount * efficiency)}u/s (average), ${
        truncate(amount)}u/s (active)`;
  }
...
Dropdowns.acidDropdown.button.addEventListener(
    'click', () => changeDropdownShow('acidDropdown', 'toggle'));
Dropdowns.oilDropdown.button.addEventListener(
    'click', () => changeDropdownShow('oilDropdown', 'toggle'));
Dropdowns.depthDropdown.button.addEventListener(
    'click', () => changeDropdownShow('depthDropdown', 'toggle'));
